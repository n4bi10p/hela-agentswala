...
async function main() {
  ...
  const existingAgents = (await registry.getAllAgents()) as Array<{ name: string }>;
  ...
  const finalAgents = (await registry.getAllAgents()) as Array<{ name: string; agentType: string }>;
  ...
  if (missing.length > 0) {
    throw new Error(`Seed incomplete. Missing agents: ${missing.map((a) => a.name).join(", ")}`);
  }
